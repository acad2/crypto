...
extern "C" {
	...
	void CUSTOM_SHAKE512(const u8 *in, u64 inLen, u8 *out, u64 outLen) { Keccak(576, 1024, in, inLen, 0x1F, out, outLen); }
...
static u64 load64(const u8 *x) { u32 i; u64 u=0; FOR(i,8) { u<<=8; u|=x[7-i]; } return u; }
static void store64(u8 *x, u64 u) { u32 i; FOR(i,8) { x[i]=u; u>>=8; } }
static void xor64(u8 *x, u64 u) { u32 i; FOR(i,8) { x[i]^=u; u>>=8; } }
#define rL(x,y) load64((u8*)s+8*(x+5*y))
#define wL(x,y,l) store64((u8*)s+8*(x+5*y),l)
#define XL(x,y,l) xor64((u8*)s+8*(x+5*y),l)
extern "C" void KeccakF1600(void *s)
{
    u32 r,x,y,i,j,Y; u8 R=0x01; u64 C[5],D;
    for(i=0; i<24; i++) {
...
void Keccak(u32 r, u32 c, const u8 *in, u64 inLen, u8 sfx, u8 *out, u64 outLen)
{
    /*initialize*/ u8 s[200]; u32 R=r/8; u32 i,b=0; FOR(i,200) s[i]=0;
