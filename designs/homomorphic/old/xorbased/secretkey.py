""" Python implementation of a cipher suitable for use with publickey.py
    Produces ciphertexts that are partially homomorphic with respect to XOR.
    Uses a non-linear key addition layer (keyed transposition), and a linear diffusing layer. """
...
def key_schedule(state):
    ...
    for round in range(1):            
        ...
        for index, byte in enumerate(state):
            total ^= byte
            byte ^= rotate_left(total, 1) ^ ((rotate_left(state[(index - 1) % state_size], 2) ^ index) & (rotate_left(state[(index + 1) % state_size], 3) ^ index))
            state[index] = byte            
            total ^= byte            
...
def encrypt(data, key, padding=None):
    ...
    padding = bytearray(urandom(8)) if padding is None else padding[:8]
    #xor_subroutine(data, padding) # not sure if this is actually necessary
    data.extend(padding)           
