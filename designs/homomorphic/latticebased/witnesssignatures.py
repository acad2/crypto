...
def unit_test():
    ...
    signature_request, validation_key, tag = generate_signature_request_on_data(pub2, data)    
    # broadcast proof of/commitment to validation key                 
    signature, _signing_key = sign_requested_data(data, signature_request, priv2, tag)
    # release signature
    # release validation key
    assert verify(data, signature, _signing_key, validation_key)
    
    signature_request, validation_key = generate_signature_request(pub1)
    signature, signing_key = sign(data, signature_request, priv1)
    assert verify(data, signature, signing_key, validation_key)    
...
def test_sign_verify_time():
    ...
    for number in range(1024):                
        #pub1, priv1 = generate_keypair()
        #pub2, priv2 = generate_keypair()    
        signature_request, validation_key, tag = generate_signature_request_on_data(pub2, message)        
        signature, signing_key = sign_requested_data(message, signature_request, priv2, tag)
        assert verify(message, signature, signing_key, validation_key), number                
    after = timer()    
    print("Time taken to sign {} {}-byte specific message: {}".format(1, len(message), (after - before) / (number + 1)))
    print("Time taken to sign {} {}-byte specific message: {}".format(number + 1, len(message), after - before))
    ...
    for number in range(1024):                
        ...
        assert verify(message, signature, signing_key, validation_key), number                
    after = timer()    
    print("Time taken to sign {} {}-byte arbitrary message: {}".format(1, len(message), (after - before) / (number + 1)))
    print("Time taken to sign {} {}-byte arbitrary message: {}".format(number + 1, len(message), after - before))
