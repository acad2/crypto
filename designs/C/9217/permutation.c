...
#define swap(c, shuf_mask) {c = _mm_shuffle_epi8(c, shuf_mask);}    
#define mix_pairs(a, b, c, d)({a += b; c += d; b ^= c; d ^= a; swap(b, shuf_mask)})
#define mix_columns(a, b, c, d) ({\
    mix_pairs(a, b, c, d); mix_pairs(a, b, c, d); mix_pairs(a, b, c, d); mix_pairs(a, b, c, d);\
    mix_pairs(a, b, c, d); mix_pairs(a, b, c, d); mix_pairs(a, b, c, d); mix_pairs(a, b, c, d);})
...
#define unmix_columns(a, b, c, d)({\
    swap(b, shuf_mask); d ^= a; b ^= c; c -= d; a -= b;\
    swap(b, shuf_mask); d ^= a; b ^= c; c -= d; a -= b;\
    swap(b, shuf_mask); d ^= a; b ^= c; c -= d; a -= b;\
    swap(b, shuf_mask); d ^= a; b ^= c; c -= d; a -= b;\
    swap(b, shuf_mask); d ^= a; b ^= c; c -= d; a -= b;\
