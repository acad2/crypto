ROUNDS = 4              # 2 ** (-differential_probability * active_sboxes) == 2 ** 
...
def generate_constant(word): # 10 instructions
...
def mix_columns(a, b, c, d): # 9 instructions
    """ Optimal 4x4 s-box implementation; Applies 32 s-boxes in parallel on the columns. """                        
    t = a    
    a = (a & b) ^ c
    c = (b | c) ^ d
    d = (d & a) ^ t
...
def round_function(a, b, c, d, round_number,  # 47 instructions
                   round_constants=[generate_constant(round_number) for round_number in range((ROUNDS * KEY_ADDITION_LAYERS) + 1)]):
    #a ^= generate_round_constant(round_number) # 11      
    a ^= round_constants[round_number]          # 2? 38 if so
    a, b, c, d = shift_and_mix(a, b, c, d, 1, 2, 3)              # 12   
    a, b, c, d = shift_and_mix(a, b, c, d, 4, 8, 12)              # 12     
    a, b, c, d = shift_and_mix(a, b, c, d, 8, 12, 16)              # 12     
    return b, c, d, a
