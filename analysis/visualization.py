...
def _print_state(inputs, bits): 
    ...
    for word in inputs:
        ...
        weight_total += weight
        print("{} ({}/bits)".format(''.join(bit for bit in format(word, 'b').zfill(bits)), weight))
...
def test_8x64_function(function, inputs, print_function=print_state_8x64):
    test_function(function, inputs, print_function)
